...
class MenuRecipeListItem extends React.Component {
	...
	editMenuRec(e) {
		...
		$.ajax({
			url: `/api/v1/menu_rec/${this.state.recipe_info.menu_rec_id}`,
			type: 'PUT',
			dataType: 'JSON',
			data: { menu_rec: { day } }
		}).done( data => {
			console.log(data)
			this.toggleEdit();
			this.setState( { recipe_info: data } )
		}).fail( data => {
			console.log(data);
		});
	}
		...
		return(
			<div className='card-panel hoverable'>
				<form onSubmit={this.editMenuRec}>
					<input type='text' defaultValue={recipe_info.day} required ref="editRecipeDay" placeholder='Recipe day' />
	...
	displayView() {
		...
		return(
			<div className='card-panel hoverable'>
				<p> {recipe_info.day} </p>
				<h3> <Link to={`recipes/${this.state.recipe_info.recipe_id}`}> {this.state.recipe_info.name} </Link> </h3>
				<p>Ingredients: {recipe_info.ingredient_count}</p>
				<button onClick={() => this.props.deleteMenuRec(this.state.id)} className='btn'>Delete</button>
